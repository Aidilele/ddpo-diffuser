...
from ddpo_diffuser.model.helpers import Losses, cosine_beta_schedule, extract, history_cover
...
class GaussianInvDynDiffusion(nn.Module):
    ...
    def p_sample(
        ...
        return model_mean + nonzero_mask * model_variance * noise, model_mean, model_variance
    ...
    def p_sample_loop(
        ...
        if return_diffusion:
            return x, torch.stack(diffusion, dim=1), torch.stack(mean, dim=1), torch.stack(variance, dim=1)
    ...
    def single_step_diffusion(self, x, obs, t, c):
        ...
        if self.predict_epsilon:
            x_recon = extract(self.sqrt_recip_alphas_cumprod, t, x.shape) * x - extract(
                self.sqrt_recipm1_alphas_cumprod, t, x.shape) * epsilon
        else:
            x_recon = epsilon
        if self.clip_denoised:
            x_recon.clamp_(-1.0, 1.0)
        ...
        model_variance = (0.5 * log_variance_clipped).exp()
